// DiningTable  Equipment Array - ID-4000

export const diningTableArray=[
    {
        id:4001,
        name:"4 Seated Dinning Table",
        img:sugarsyrup,
        des:"Enhance your dining area with Kalam Kitchen's 4 Seated Dining Table, designed for intimate and efficient dining experiences. Our table features durable construction, compact size, and a stylish design. Ideal for cafes, small restaurants, and casual dining areas, it provides a sturdy and space-saving dining solution. The robust build ensures long-lasting performance, while the smooth surface enhances cleanliness and maintenance. Trust our 4 Seated Dining Table for reliable and attractive dining arrangements, improving guest comfort and dining efficiency in smaller spaces.",
        keyFeatures: [
            {
              title: "Compact Design",
              value: "Perfect for smaller spaces, seating up to four people comfortably.",
            },
            {
              title: "High-Quality Materials",
              value:
                "Constructed from durable materials like stainless steel or wood.",
            },
            {
              title: "Stylish Appearance",
              value:
                "Modern and sleek design that fits well in any dining setting.",
            },
            {
              title: "Stable Construction",
              value:
                "Features sturdy legs and a robust frame for stability.",
            },
            {
              title: "Easy to Maintain",
              value:
                "Smooth surface for effortless cleaning.",
            },
          ],
          smartChoice: [
            {
              title: "Space-Efficient",
              value: "Ideal for compact dining areas or small households.",
            },
            {
              title: "Durable and Stylish",
              value: "Combines longevity with a modern aesthetic.",
            },
            {
              title: "Low Maintenance",
              value: "Easy to clean and keep in pristine condition.",
            },
          ],
         type:"DINING TABLE EQUIPMENTS",
         ratingStar:5,
         slug:"4-Seated-Dinning-Tablet",
         stock:"IN STOCK",
         reviewNumber:"4.8",
         reviewCound:"1002"
    },
    {
        id:4002,
        name:"8 Seated Dinning Table",
        img:sugarsyrup,
        des:"Kalam Kitchen's 8 Seated Dining Table is designed for spacious and comfortable dining experiences in commercial settings. Our table features durable construction, ample seating capacity, and an elegant design. Ideal for restaurants, cafeterias, and dining halls, it provides a sturdy and stylish dining solution. The robust build ensures long-lasting performance, while the smooth surface enhances cleanliness and maintenance. Trust our 8 Seated Dining Table for reliable and attractive dining arrangements, improving guest comfort and dining efficiency in your establishment.",
        keyFeatures: [
            {
              title: "Spacious Design",
              value:
                "Accommodates up to eight people, perfect for family gatherings and social events.",
            },
            {
              title: "Durable Construction",
              value:
                "Made from high-quality materials such as stainless steel or wood for long-lasting use.",
            },
            {
              title: "Elegant Finish",
              value:
                "Sleek and polished surface adds a touch of sophistication to any space.",
            },
            {
              title: "Sturdy Legs",
              value:
                "Strong and stable legs ensure durability and safe usage.",
            },
            {
              title: "Easy to Clean",
              value:
                "Smooth surface design allows quick and effortless cleaning.",
            },
          ],
      smartChoice: [
            {
              title: "Spacious Seating",
              value:
                "Ideal for large families or groups, providing ample space for comfortable dining.",
            },
            {
              title: "Durable and Elegant",
              value:
                "Built for long-lasting use with a refined finish that enhances any dining area.",
            },
            {
              title: "Easy Maintenance",
              value:
                "Simple to clean, ensuring long-lasting beauty and functionality.",
            },
          ],
         type:"DINING TABLE EQUIPMENTS",
         ratingStar:5,
         slug:"8-Seated-Dinning-Tablet",
         stock:"IN STOCK",
         reviewNumber:"4.6",
         reviewCound:"1010"
    },
    {
        id:4003,
        name:"10 Seated Dinning Table",
        img:sugarsyrup,
        des:"Kalam Kitchen's 10 Seated Dining Table is designed for large group dining in commercial settings. Our table features durable construction, generous seating capacity, and an elegant design. Ideal for banquet halls, cafeterias, and large restaurants, it provides a sturdy and stylish dining solution. The robust build ensures long-lasting performance, while the smooth surface enhances cleanliness and maintenance. Trust our 10 Seated Dining Table for reliable and attractive dining arrangements, improving guest comfort and dining efficiency for larger groups.",
        keyFeatures: [
            {
              title: "Generous Seating",
              value:
                "Accommodates up to ten people, perfect for large gatherings and events.",
            },
            {
              title: "Premium Materials",
              value:
                "Made from top-quality materials for durability and elegant appearance.",
            },
            {
              title: "Elegant Design",
              value:
                "Polished and sophisticated finish that complements any interior decor.",
            },
            {
              title: "Robust Frame",
              value:
                "Strong legs and solid frame ensure excellent stability and support.",
            },
            {
              title: "Hygienic Surface",
              value:
                "Easy-to-clean surface ensures hygiene and everyday convenience.",
            },
          ],
          smartChoice: [
            {
              title: "Large Capacity",
              value:
                "Perfect for big families or social gatherings, offering plenty of space.",
            },
            {
              title: "Elegant and Durable",
              value:
                "Combines an elegant look with durable construction.",
            },
            {
              title: "Hygienic and Convenient",
              value:
                "Easy to clean, maintaining a high standard of hygiene.",
            },
          ],
         type:"DINING TABLE EQUIPMENTS",
         ratingStar:5,
         slug:"10-Seated-Dinning-Tablet",
         stock:"IN STOCK",
         reviewNumber:"4.6",
         reviewCound:"1010"
    },
    {
        id:4004,
        name:"Restaurant Table & Chairs",
        img:sugarsyrup,
        des:"Enhance your dining area with Kalam Kitchen's Restaurant Table & Chairs, designed for comfort and style. Our set features durable construction, ergonomic seating, and a sleek design. Ideal for restaurants, cafes, and dining halls, it provides a sturdy and stylish dining solution. The robust build ensures long-lasting performance, while the smooth surfaces enhance cleanliness and maintenance. Trust our Restaurant Table & Chairs for reliable and attractive dining arrangements, improving guest comfort and dining efficiency in your establishment.",
        keyFeatures: [
            {
              title: "Commercial Grade",
              value:
                "Designed specifically for restaurant use with strong and durable construction.",
            },
            {
              title: "Stylish Design",
              value:
                "Modern and appealing look that enhances the dining experience.",
            },
            {
              title: "Comfortable Seating",
              value:
                "Ergonomic design ensures comfort during extended use.",
            },
            {
              title: "High-Quality Materials",
              value:
                "Made from durable stainless steel or wood for long-lasting performance.",
            },
            {
              title: "Easy to Clean",
              value:
                "Smooth surfaces allow quick and easy cleaning.",
            },
          ],
        smartChoice: [
            {
              title: "Durable and Reliable",
              value:
                "Built to withstand the rigors of a busy restaurant environment.",
            },
            {
              title: "Stylish and Comfortable",
              value:
                "Enhances the dining experience with a stylish and comfortable design.",
            },
            {
              title: "Low Maintenance",
              value:
                "Easy to clean and maintain, ensuring long-lasting use.",
            },
          ],
         type:"DINING TABLE EQUIPMENTS",
         ratingStar:5,
         slug:"Restaurant-Table-Chairs",
         stock:"IN STOCK",
         reviewNumber:"4.5",
         reviewCound:"620"
    },
    {
        id:4005,
        name:"Restaurant Table / Chairs with Granite Top",
        img:sugarsyrup,
        des:"Upgrade your dining area with Kalam Kitchen's Restaurant Table / Chairs with Granite Top, designed for luxury and durability. Our set features a sturdy construction, elegant granite top, and ergonomic seating. Ideal for high-end restaurants, cafes, and dining halls, it provides a stylish and long-lasting dining solution. The robust build ensures reliable performance, while the easy-to-clean surfaces enhance maintenance. Trust our Restaurant Table / Chairs with Granite Top for reliable and attractive dining arrangements, elevating guest comfort and dining experience.",
        keyFeatures: [
            {
              title: "Elegant Granite Top",
              value:
                "Features a durable and attractive granite top that adds a touch of luxury.",
            },
            {
              title: "Sturdy Construction",
              value:
                "Made from high-quality materials for strength and stability.",
            },
            {
              title: "Comfortable Seating",
              value:
                "Chairs designed to provide comfort during meals.",
            },
            {
              title: "Modern Design",
              value:
                "Sleek and contemporary look suitable for upscale dining settings.",
            },
            {
              title: "Easy Maintenance",
              value:
                "Granite top is easy to clean and maintain.",
            },
          ],
        smartChoice: [
            {
              title: "Luxurious Finish",
              value:
                "Granite top adds elegance and sophistication to any dining area.",
            },
            {
              title: "Durable and Sturdy",
              value:
                "Built with high-quality materials for long-lasting strength and stability.",
            },
            {
              title: "Easy to Clean",
              value:
                "Smooth granite surface ensures simple and hassle-free maintenance.",
            },
          ],
         type:"DINING TABLE EQUIPMENTS",
         ratingStar:5,
         slug:"Restaurant-Table-Chairs-with-Granite-Top",
         stock:"IN STOCK",
         reviewNumber:"4.7",
         reviewCound:"1620"
    },
    {
        id:4006,
        name:"MS Table Frame with Chair",
        img:sugarsyrup,
        des:"Optimize your dining space with Kalam Kitchen's MS Table Frame with Chair, designed for durability and style. Our table frame and chair set features sturdy mild steel construction, ergonomic design, and a sleek finish. Ideal for restaurants, cafes, and dining halls, it provides a reliable and comfortable seating solution. The robust build ensures long-lasting performance, while the easy-to-clean surfaces enhance maintenance. Trust our MS Table Frame with Chair for reliable and attractive dining arrangements, improving guest comfort and dining efficiency in your establishment.",
        keyFeatures: [
            {
              title: "Sturdy Construction",
              value:
                "Made with mild steel (MS) for robust and reliable support.",
            },
            {
              title: "Modern Design",
              value:
                "Sleek and contemporary appearance suitable for various settings.",
            },
            {
              title: "Comfortable Seating",
              value:
                "Comes with chairs designed for comfort and support.",
            },
            {
              title: "Versatile Use",
              value:
                "Ideal for dining rooms, cafes, and restaurants.",
            },
            {
              title: "Low Maintenance",
              value:
                "Easy to clean and maintain, ensuring long-lasting use.",
            },
          ],
        smartChoice :[
            {
              title: "Robust and Reliable",
              description: "Built with strong MS material for long-term durability."
            },
            {
              title: "Modern Aesthetic",
              description: "Adds a contemporary touch to any dining space."
            },
            {
              title: "Comfort and Convenience",
              description: "Comfortable seating with low maintenance requirements."
            }
          ],
         type:"DINING TABLE EQUIPMENTS",
         ratingStar:5,
         slug:"MS-Table-Frame-with-Chair",
         stock:"IN STOCK",
         reviewNumber:"4.7",
         reviewCound:"1620"
    },
    {
        id:4007,
        name:"Canteen Dining Table",
        img:sugarsyrup,
        des:"Kalam Kitchen's Canteen Dining Table is designed for robust and efficient dining solutions in busy canteens. Our table features durable construction, ample seating capacity, and a practical design. Ideal for schools, colleges, and corporate canteens, it provides a reliable and sturdy dining solution. The robust build ensures long-lasting performance, while the easy-to-clean surfaces enhance maintenance. Trust our Canteen Dining Table for reliable and efficient dining arrangements, improving guest comfort and operational efficiency in high-traffic dining areas.",
        keyFeatures : [
            {
              title: "Large Seating Capacity",
              description: "Designed to accommodate multiple people, perfect for canteens."
            },
            {
              title: "Durable Construction",
              description: "Made from robust materials for frequent use."
            },
            {
              title: "Easy to Clean",
              description: "Smooth surfaces for quick and easy cleaning."
            },
            {
              title: "Comfortable Seating",
              description: "Ergonomically designed for comfort during meals."
            },
            {
              title: "Practical Design",
              description: "Features a simple and functional design suitable for canteens."
            }
          ],
        smartChoice: [
            {
              title: "Spacious and Durable",
              description: "Provides ample seating with strong construction."
            },
            {
              title: "Comfortable and Practical",
              description: "Designed for comfort and practicality."
            },
            {
              title: "Easy Maintenance",
              description: "Simple to clean and maintain."
            }
          ],
         type:"DINING TABLE EQUIPMENTS",
         ratingStar:5,
         slug:"Canteen-Dining-Table",
         stock:"IN STOCK",
         reviewNumber:"4.5",
         reviewCound:"1520"
    },
    {
        id:4008,
        name:"SS Standing Table",
        img:sugarsyrup,
        des:"Optimize your space with Kalam Kitchen's SS Standing Table, designed for modern and efficient dining and working environments. Our table features durable stainless steel construction, ergonomic height, and a sleek design. Ideal for cafes, restaurants, and casual dining areas, it provides a sturdy and stylish standing solution. The robust build ensures long-lasting performance, while the smooth surface enhances cleanliness and maintenance. Trust our SS Standing Table for reliable and attractive standing arrangements, improving guest comfort and space utilization.",
         keywords : [
            {
              title: "Sturdy Construction",
              description: "Made from high-quality stainless steel for durability."
            },
            {
              title: "Modern Design",
              description: "Sleek and contemporary look suitable for various settings."
            },
            {
              title: "Versatile Use",
              description: "Ideal for standing dining, bars, and casual seating areas."
            },
            {
              title: "Stable Base",
              description: "Features a stable base for secure and safe use."
            },
            {
              title: "Easy to Clean",
              description: "Designed for easy maintenance with smooth surfaces."
            }
          ],
         smartChoice : [
            {
              title: "Durable and Stylish",
              description: "Built to last with a modern design."
            },
            {
              title: "Versatile and Practical",
              description: "Suitable for a variety of settings."
            },
            {
              title: "Low Maintenance",
              description: "Easy to clean, ensuring long-term use."
            }
          ],
         type:"DINING TABLE EQUIPMENTS",
         ratingStar:5,
         slug:"SS-Standing-Table",
         stock:"IN STOCK",
         reviewNumber:"4.5",
         reviewCound:"1520"
    },
    {
        id:4009,
        name:"SS Designer Chair",
        img:sugarsyrup,
        des:"Upgrade your dining area with Kalam Kitchen's SS Designer Chair, designed for luxury and durability. Our chair features high-quality stainless steel construction and an ergonomic design for maximum comfort. Ideal for upscale restaurants, cafes, and dining halls, it provides a stylish and durable seating solution. The robust build ensures long-lasting performance, while the sleek design enhances the aesthetic appeal. Trust our SS Designer Chair for reliable and attractive seating arrangements, elevating guest comfort and dining experience.",
        keyFeatures : [
            {
              title: "Unique Design",
              description: "Features a distinctive and stylish design that stands out."
            },
            {
              title: "Durable Construction",
              description: "Made from high-quality stainless steel for longevity."
            },
            {
              title: "Comfortable",
              description: "Ergonomically designed for comfortable seating."
            },
            {
              title: "Versatile Use",
              description: "Suitable for various settings including homes, cafes, and restaurants."
            },
            {
              title: "Easy to Maintain",
              description: "Smooth surface for easy cleaning."
            }
          ],
        smartChoice : [
            {
              title: "Stylish and Unique",
              description: "Adds a unique touch to any dining area."
            },
            {
              title: "Durable and Comfortable",
              description: "Built to last with a focus on comfort."
            },
            {
              title: "Easy to Clean",
              description: "Low maintenance, ensuring lasting beauty and functionality."
            }
          ],
         type:"DINING TABLE EQUIPMENTS",
         ratingStar:5,
         slug:"SS-Designer-Chair",
         stock:"IN STOCK",
         reviewNumber:"4.5",
         reviewCound:"1520"
    },
    {
        id:4010,
        name:"Rubber Wood Chair with SS Frame",
        img:sugarsyrup,
        des:"Elevate your dining space with Kalam Kitchen's Rubber Wood Chair with SS Frame, designed for comfort and elegance. Our chair features a sturdy stainless steel frame and a comfortable rubber wood seat and backrest. Ideal for restaurants, cafes, and dining halls, it provides a stylish and durable seating solution. The robust build ensures long-lasting performance, while the smooth surfaces enhance cleanliness and maintenance. Trust our Rubber Wood Chair with SS Frame for reliable and attractive seating arrangements, improving guest comfort and dining experience.",
        keyFeatures : [
            {
              title: "Sturdy Construction",
              description: "Combines the strength of stainless steel with the elegance of rubber wood."
            },
            {
              title: "Comfortable Seating",
              description: "Ergonomically designed for maximum comfort."
            },
            {
              title: "Elegant Design",
              description: "Features a stylish and modern look suitable for various settings."
            },
            {
              title: "Durable Materials",
              description: "Built to withstand frequent use with high-quality materials."
            },
            {
              title: "Easy to Clean",
              description: "Smooth surfaces for easy maintenance."
            }
          ],
          smartChoice : [
            {
              title: "Strong and Elegant",
              description: "Combines durability with a refined appearance."
            },
            {
              title: "Comfortable and Stylish",
              description: "Designed for comfort with a modern look."
            },
            {
              title: "Low Maintenance",
              description: "Easy to clean, ensuring long-lasting use."
            }
          ],
         type:"DINING TABLE EQUIPMENTS",
         ratingStar:5,
         slug:"Rubber-Wood-Chair-with-SS-Frame",
         stock:"IN STOCK",
         reviewNumber:"4.5",
         reviewCound:"1520"
    },
]